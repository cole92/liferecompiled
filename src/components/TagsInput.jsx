...
const TagsInput = ({ tags, setTags, disabled = false }) => {
  ...
  const renderFilteredTags = () => {
    if (filteredForRender.length === 0) {
      return (
        <div className="absolute left-0 top-full z-50 mt-2 w-full rounded-xl border border-zinc-800 bg-zinc-950 p-3 shadow-md">
    ...
    return (
      <div className="absolute left-0 top-full z-50 mt-2 w-full max-h-72 overflow-auto ui-scrollbar rounded-xl border border-zinc-800 bg-zinc-950 shadow-md">
        <div className="p-3">
          {filteredForRender.map(({ name, tags: list }, idx) => (
            <div key={name} className={idx === 0 ? "" : "mt-4"}>
              <h5 className="sticky top-0 z-10 -mx-3 mb-2 border-b border-zinc-800 bg-zinc-950 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-zinc-400">
                {name}
              </h5>
